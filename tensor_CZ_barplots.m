...
RMSa_ts_oos(R) = rms(a_ts_oos,'all')
RMSa_hml_oos(R) = rms(a_hml_oos,'all')
RMSa_hal_oos(R) = rms(a_hal_oos,'all')
idiovar_ts_oos(R) = mean(var(a_ts_oos,0,1)./var(Y_1(TT+1:end,:),0,1))
idiovar_hml_oos(R) = mean(var(a_hml_oos,0,1)./var(Y_1(TT+1:end,:),0,1))
idiovar_hal_oos(R) = mean(var(a_hal_oos,0,1)./var(Y_1(TT+1:end,:),0,1))
SR_ts_oos(R) = mean(port_ts)/std(port_ts)
SR_hml_oos(R) = mean(port_hml)/std(port_hml)
SR_hal_oos(R) = mean(port_hal)/std(port_hal)
end

%% bar plot
x1 = categorical({'Tensor'});
x2 = categorical({'Tensor','HML','HAL'});
x2 = reordercats(x2,{'Tensor','HML','HAL'});
ax1 = subplot(3,2,1);
bar(x1,RMSa_ts);
title('RMS_\alpha (in-sample)');

ax2 = subplot(3,2,3);
bar(x1,idiovar_ts);
title('\sigma_e^2 (in-sample)');

ax3 = subplot(3,2,5);
bar(x1,SR_ts);
title('SR (in-sample)');
lg = legend(sprintfc('factor %d',1:10));

ax4 = subplot(3,2,2);
bar(x2,[RMSa_ts_oos;RMSa_hml_oos;RMSa_hal_oos]);
title('RMS_\alpha (out-of-sample)');

ax5 = subplot(3,2,4);
bar(x2,[idiovar_ts_oos;idiovar_hml_oos;idiovar_hal_oos]);
title('\sigma_e^2 (out-of-sample)');

ax6 = subplot(3,2,6);
bar(x2,[SR_ts_oos;SR_hml_oos;SR_hal_oos]);
title('SR (out-of-sample)');
