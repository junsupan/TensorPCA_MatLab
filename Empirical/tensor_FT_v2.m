...
clear;clc;
rng(23,'twister');
Data1 = readtable('FT90_20.csv','ReadVariableNames',true);
...
RF = table2array(Data2(first2:last2,"RF")).*0.01;

Y = NaN(T,J,N);
for n=1:N
    Y(:,:,n) = Data1{(n-1)*T+1:n*T,2:11};
end
% excess return
Y = Y - repmat(RF,1,J,N);
...
s0 = (s1+s2+s3)/3;
[s_ts,~] = findsigma(Y,{F_hat,M_hat,L_hat},s0);
...
%     R2_ts(r) = 1 - var(Y_e,0,'all')/var(Y,0,'all');
    ...
    AIC_ts(r) = NN*log(sum(Y_e.^2,'all')/NN) + r*(N+J+T)*log(NN);
end
...
for r=1:R
    ...
    AIC_als(r) = NN*log(sum(Yr.^2,'all')/NN) + r*(N+J+T)*log(NN);
end
...
%     R2_tw(r) = 1 - var(Y_1_e,0,'all')/var(Y,0,'all');
    ...
    AIC_tw(r) = NN*log(sum(Y_1_e.^2,'all')/NN) + r*(N*J+T)*log(NN);
end
% Model complexity
